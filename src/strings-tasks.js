...
function formatTime(minutes, seconds) {
  ...
  const minToStr = String(minutes).padStart(2, '0');
  const secToStr = String(seconds).padStart(2, '0');
  const time = `${minToStr}:${secToStr}`;
...
function isPalindrome(str) {
  const strToLowerCase = str.toLowerCase().replace(/[^a-z0-9]/g, '');
  const reversStr = strToLowerCase.split('').reverse().join('');
  return strToLowerCase === reversStr;
}
...
function findLongestWord(sentence) {
  return sentence
    .trim()
    .split(/\s+/)
    .reduce((long, curr) => (curr.length > long.length ? curr : long));
}
...
function invertCase(str) {
  return str
    .split('')
    .map((dig) =>
      dig === dig.toLowerCase() ? dig.toUpperCase() : dig.toLowerCase()
    )
    .join('');
}
...
function encodeToRot13(str) {
  ...
  return str.replace(
    /[a-z]/gi,
    (dig) => rotAlphabet[originalAlphabet.indexOf(dig)]
  );
}
